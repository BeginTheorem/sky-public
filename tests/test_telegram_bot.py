...
class TelegramBotTests(unittest.TestCase):
    def test_chunks_and_redaction(self):
        self.assertEqual(chunks("x" * 10, 4), ["xxxx", "xxxx", "xx"])
        ...
        formatted_time = format_runtime_record({"timestamp": _iso(9652.0), "kind": "finish_report", "payload": {}})
    ...
    def test_send_queues_only_authorized_private_message(self):
        api = FakeAPI()
        with tempfile.TemporaryDirectory() as directory:
            state = Path(directory) / "state.sqlite3"
            bot = PrivateBot(cast(TelegramAPI, api), state, 7, 8, "skynet.service")
            bot.process({"message": {"from": {"id": 7}, "chat": {"id": 8}, "text": "/send hello"}})
            self.assertIn("поставлено", api.sent[0][1])
            from skynet.store import StateStore
            store = StateStore(state, read_only=True)
            try:
                rows = store.pending_inbox()
            finally:
                store.close()
            self.assertEqual(rows[0]["payload"]["text"], "hello")
    ...
    def test_api_call_filters_ipv6_when_using_socks_proxy(self):
        api = TelegramAPI("token", "socks5://127.0.0.1:1080")
        response = Mock()
        ...
        with patch("skynet.telegram_bot.request.urlopen", return_value=response), patch("skynet.telegram_bot.socks.set_default_proxy") as set_proxy, patch("skynet.telegram_bot.socks.socksocket"):
            api.call("getMe")
        self.assertEqual(set_proxy.call_args_list[0].args, (socks.SOCKS5, "127.0.0.1", 1080))
    ...
    def test_api_call_preserves_hostname_for_socks_remote_dns(self):
        api = TelegramAPI("token", "socks5://127.0.0.1:1080")
        response = Mock()
        response.__enter__ = Mock(return_value=response)
        response.__exit__ = Mock(return_value=False)
...
class TelegramOwnerChannelTests(unittest.TestCase):
    ...
    def test_alerts_lists_pending_most_recent_first(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            store.connection.execute("UPDATE alerts SET last_seen_at=? WHERE kind='first'", (_iso(0.0),))
